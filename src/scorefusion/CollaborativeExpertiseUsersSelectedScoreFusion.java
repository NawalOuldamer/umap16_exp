 ...
 *
 */
public class CollaborativeExpertiseUsersSelectedScoreFusion {
	...
	public static void main(String[] args) throws IOException {
		...
		while(i.hasNext())		
		{
			...
			/* select les scores expertise entre users */
				...
				if(liste_top_k_doc.contains(vect[2])){
					if(map_user_score_expertise.containsKey(vect[0])){
						if(map_doc_score.containsKey(vect[2])){
							...
							map_doc_score.put(vect[2], map_doc_score.get(vect[2]) + Double.parseDouble((vect[4]))/map_user_max_score_normalisation.get(vect[0]) * map_user_score_expertise.get(vect[0]));
						}
						else {
							map_doc_score.put(vect[2], Double.parseDouble((vect[4]))/map_user_max_score_normalisation.get(vect[0]) * map_user_score_expertise.get(vect[0]));
						}
					}
				}
			}
			...
			FileWriter fichier_resultat = new FileWriter("./collaborative_part/expertise_resultats_fusion/users_selected/"+number_of_topics+"_topics/rsv_collaboratif_number_of_topics_"+number_of_topics+"_query_"+courant.getChild("NUM").getText()+".res");
			int rank = 0;
