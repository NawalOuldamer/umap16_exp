 ...
 *
 */
public class NotFilteredExpCollaborativeSelectedUsersScore {
	...
	public static void main(String[] args) throws IOException {
		...
		while(i.hasNext())		
		{
			...
			while (s_user.hasNextLine()) {
				String line = s_user.nextLine();
				String [] vect = line.split(" ");
				if(liste_top_k_doc.contains(vect[2]) && map_user_score_expertise.containsKey(vect[0])){
					if(map_doc_score.containsKey(vect[2])){
						...
						map_doc_score.put(vect[2], (map_doc_score.get(vect[2]) +
								...
								* map_user_score_expertise.get(vect[0])))/2); // faire la moyenne
					}
					else {
			...
			FileWriter fichier_resultat = new FileWriter("./collaborative_part/profile_not_filtred/expertise_weighted_users_filtred/"+number_of_topics+"_topics/rsv_collaboratif_prfile_not_filtered"+courant.getChild("NUM").getText()+".res");
			int rank = 0;
