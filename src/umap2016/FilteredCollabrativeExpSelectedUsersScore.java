 ...
 *
 */
public class FilteredCollabrativeExpSelectedUsersScore {
	...
	public static void main(String[] args) throws IOException {
		...
		while(i.hasNext())		
		{
			...
			// le score expertise de chaque user par rapport a la requete courante 
			
			/* select les scores expertise users */
			...
			while (s_expertise.hasNextLine() && trouve== false) {
				...
				if (!line.startsWith("#")) {
					...
					if(Double.parseDouble(vect[3])!=0.0 && liste_users_selected.contains(vect[2])){
			...
			map = sortByComparator(map_user_score_expertise, DESC);
				...
				if(liste_top_k_doc.contains(vect[2])){
					if(map_user_score_expertise.containsKey(vect[0])){
						if(map_doc_score_with_expertise.containsKey(vect[2])){
							map_doc_score_with_expertise.put(vect[2], (map_doc_score_with_expertise.get(vect[2]) + Double.parseDouble((vect[4]))/map_user_score_document_max.get(vect[0]) * map_user_score_expertise.get(vect[0]))/2);
							
						}
						else {
							map_doc_score_with_expertise.put(vect[2], Double.parseDouble((vect[4]))/map_user_score_document_max.get(vect[0]) * map_user_score_expertise.get(vect[0]));
			...
			FileWriter fichier_resultat_with_expertise = new FileWriter("./collaborative_part/profile_filtred/fusion_expertise_users_selected/"+number_of_topics+"_topics/rsv_collaboratif_number_of_topics_"+number_of_topics+"_query_"+courant.getChild("NUM").getText()+".res");
			int rank = 0;
